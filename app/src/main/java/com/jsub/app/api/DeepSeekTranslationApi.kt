 ...
 * DeepSeek 翻译API实现
 *
 * 使用DeepSeek的LLM Chat API进行日文到中文的翻译。
 * 利用大语言模型的上下文理解能力，翻译质量通常优于传统翻译API。
 *
 * API文档: https://platform.deepseek.com/docs
 *
 * @param apiKey DeepSeek API Key（从 https://platform.deepseek.com 获取）
 */
class DeepSeekTranslationApi(
    ...
    companion object {
        private const val TAG = "DeepSeekTranslationApi"
    ...
    data class ChatResponse(
        ...
        @SerialName("error") val error: DeepSeekError? = null,
    ...
    override suspend fun translate(japaneseText: String): String = withContext(Dispatchers.IO) {
        ...
        if (apiKey.isBlank()) {
            ...
            return@withContext "[请在设置中配置DeepSeek API Key]"
        }
